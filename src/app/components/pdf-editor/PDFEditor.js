...
export default function PDFEditor({ file, onReset, mode = 'default', options = {} }) {
  ...
  const { incrementCropped } = useStats();
  ...
  const handleMouseMove = (e) => {
    ...
    const pdfCanvas = pageContainer.querySelector('.react-pdf__Page__canvas');
    const pdfRect = pdfCanvas ? pdfCanvas.getBoundingClientRect() : rect;

    // Calculate position relative to the page container
    let x = Math.max(0, Math.min(e.clientX - rect.left, rect.width));
    let y = Math.max(0, Math.min(e.clientY - rect.top, rect.height));

    // If we have a PDF canvas, constrain to its boundaries
    if (pdfCanvas) {
      const pdfLeft = pdfRect.left - rect.left;
      const pdfTop = pdfRect.top - rect.top;
      const pdfRight = pdfLeft + pdfRect.width;
      const pdfBottom = pdfTop + pdfRect.height;

      // Only constrain if the point is outside the PDF area
      if (x < pdfLeft || x > pdfRight || y < pdfTop || y > pdfBottom) {
        x = Math.max(pdfLeft, Math.min(x, pdfRight));
        y = Math.max(pdfTop, Math.min(y, pdfBottom));
      }
    }
    ...
    if (activeHandle) {
      ...
      if (pdfCanvas) {
        ...
        // For 'move' operation, maintain selection size but constrain position
        if (activeHandle === 'move') {
          // If selection would go outside left boundary
          if (newX < pdfLeft) {
            newX = pdfLeft;
          }

          // If selection would go outside top boundary
          if (newY < pdfTop) {
            newY = pdfTop;
          }

          // If selection would go outside right boundary
          if (newX + newWidth > pdfRight) {
            newX = pdfRight - newWidth;
          }

          // If selection would go outside bottom boundary
          if (newY + newHeight > pdfBottom) {
            newY = pdfBottom - newHeight;
          }
        } else {
          // For resize operations, apply different constraints

          // For left-side handles (topLeft, bottomLeft, left)
          if (activeHandle === 'topLeft' || activeHandle === 'bottomLeft' || activeHandle === 'left') {
            if (newX < pdfLeft) {
              // Maintain width by adjusting newX only
              newX = pdfLeft;
            }
          }

          // For top handles (topLeft, topRight, top)
          if (activeHandle === 'topLeft' || activeHandle === 'topRight' || activeHandle === 'top') {
            if (newY < pdfTop) {
              // Maintain height by adjusting newY only
              newY = pdfTop;
            }
          }

          // For right-side handles (topRight, bottomRight, right)
          if (activeHandle === 'topRight' || activeHandle === 'bottomRight' || activeHandle === 'right') {
            if (newX + newWidth > pdfRight) {
              // Maintain width by capping it
              newWidth = pdfRight - newX;
            }
          }

          // For bottom handles (bottomLeft, bottomRight, bottom)
          if (activeHandle === 'bottomLeft' || activeHandle === 'bottomRight' || activeHandle === 'bottom') {
            if (newY + newHeight > pdfBottom) {
              // Maintain height by capping it
              newHeight = pdfBottom - newY;
            }
          }
        }
      } else {
        // Fallback to container boundaries if PDF canvas not found
  ...
  const downloadCroppedPDF = () => {
    ...
    // Increment the cropped counter when a PDF is downloaded
    incrementCropped();
  };
  ...
  // Effect to handle mode-specific initialization
  useEffect(() => {
    if (mode === 'meesho' && pdfFile) {
      // Auto-process Meesho labels when in Meesho mode
      applyMeeshoCrop();
    }
  }, [mode, pdfFile]);

  // Specialized function for Meesho label cropping
  const applyMeeshoCrop = async () => {
    if (!pdfFile) return;
    ...
    try {
      console.log("Starting Meesho label crop process");
      ...
      // If sorting by SKU is enabled, we would need to extract and sort the pages
      // This is a placeholder for the actual implementation
      let pagesToProcess = Array.from({ length: pageCount }, (_, i) => i);

      if (options && options.sortBySKU) {
        console.log("Sorting by SKU IDs enabled");
        // In a real implementation, you would:
        // 1. Extract text from each page to find SKU IDs
        // 2. Sort the pages based on the SKU IDs
        // 3. Process the pages in the sorted order

        // For now, we'll just log that sorting is enabled
        console.log("Pages would be sorted by SKU IDs here");
      }

      // Process pages in the determined order
      for (const pageIndex of pagesToProcess) {
        ...
        console.log(`Page ${pageIndex + 1} dimensions:`, { originalWidth, originalHeight });

        // Meesho label specific dimensions
        // Fixed width of 428pt with 9pt margins on both sides
        const cropWidth = 428; // Fixed width for Meesho labels

        // Center the crop horizontally
        const cropX = Math.max(0, (originalWidth - cropWidth) / 2);

        // For height, we start from the top (y=0) and extend to cover the full height
        // In a real implementation, you would detect the Tax Invoice section
        const cropY = 0; // Start from the top
        const cropHeight = originalHeight; // Use full height for now

        console.log(`Applying crop to page ${pageIndex + 1}:`, { cropX, cropY, cropWidth, cropHeight });
      ...
      // Save the cropped PDF
      const croppedPdfBytes = await croppedPdfDoc.save();
      console.log("Meesho cropped PDF created with size:", croppedPdfBytes.byteLength);
      ...
      const croppedData = {
        file: croppedPdfBlob,
        url: croppedPdfUrl,
        name: `meesho-cropped-${pdfFile.name || 'document.pdf'}`,
        size: croppedPdfBlob.size,
        numPages: pageCount,
        sortedBySKU: options && options.sortBySKU
      };

      console.log("Setting Meesho cropped PDF data:", croppedData);
      setCroppedPdfData(croppedData);
      ...
      console.error("Error cropping Meesho label:", error);
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="bg-white border border-gray-200 rounded-lg shadow-sm p-4 space-y-3">
                      ...
                      <div className="flex items-center">
                        <div className="bg-blue-100 p-2 rounded-md mr-3">
                          <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-blue-700" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                            <path d="M16.707 5.293a1 1 0 010 1.414L9.414 10l3.293 3.293a1 1 0 01-1.414 1.414l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 0z" clipRule="evenodd" />
                            <path d="M9 12h6"></path>
                            <path d="M9 16h6"></path>
                            <path d="M9 8h6"></path>
                          </svg>
