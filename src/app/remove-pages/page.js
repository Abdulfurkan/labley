...
import { getTransferredPDFFile } from '../lib/utils';
...
export default function RemovePages() {
  ...
  useEffect(() => {
    // First check for shared PDF from context (higher priority)
    if (sharedPDF) {
      console.log('Received shared PDF in Remove Pages:', sharedPDF);
    ...
    // Fall back to the older transfer mechanism for backward compatibility
    else {
      const transferredFile = getTransferredPDFFile();
      if (transferredFile) {
        // Create a new blob URL for the transferred file
        fetch(transferredFile.url)
          .then(response => response.blob())
          .then(blob => {
            const file = new File([blob], transferredFile.name, {
              type: transferredFile.type,
              lastModified: transferredFile.lastModified
            });
            handleFileSelected({ file, name: transferredFile.name });
          })
          .catch(error => {
        ...
        <div className="text-center mb-10 px-4">
          ...
          <div className="max-w-2xl mx-auto relative">
            <p className="text-gray-600 text-lg leading-relaxed mb-6">
              Easily delete unwanted pages from your PDF documents.
              <span className="hidden sm:inline"> Select specific pages or page ranges to remove with just a few clicks.</span>
            </p>
